...
/**
 * Diese Klasse speichert Komponente in einer CSV datei und kann diese auch
 * lesen. Zudem kann sie auch Gebaeude speichern und lesen.Sie bietet außerdem
 * Methoden zum löschen und Erstellen von CSV Dateien
 * 
 * 
 * @author Davis Fröse
...
public class CSVReadWrite {
	...
	/**
	 * Erstellt eine CSV Datei mit den Geräten und schreibt neue Geräte in diese
	 * Datei (data.csv)
	 * 
	 * @author Davis Fröse
	 * @param ArrayList
	...
	public static void writeCSVGebaeude(List<Gebaeude> gebaeude) {
		...
		try {
			...
			System.out.println("CSV file was created updated !!!");
	...
	/**
	 * Liest die Datei data.csv (komponenten) aus, erstellt Komponenten-Objekte
	 * aus den Daten der CSV und erstellt eine ArrayList<Komponente> aus den
	...
	public static List<Komponente> readCSV() {
		...
		for (ArrayList l : data) {
			...
			// System.out.println();
			komponenten.add(new Komponente(object.get(0), object.get(1), object
					.get(2), object.get(3)));
		}
	...
	/**
	 * Liest die CSV Datei mit den Komponenten aus, erstellt Gebäude und
	 * Raum-Objekte aus den Daten der CSV und erstellt eine ArrayList<Gebaeude>
	 * mit den entsprechenden Daten Die Raum-Objekete zu jedem Gebäude werden in
	 * der CSV Date durch [] gekennzeichnet.
	 * 
	 * @author Marco D'Addona
	 * @param
	...
	public static List<Gebaeude> readCSVGebaeude() {
		...
		try {
			...
			while ((readline = br.readLine()) != null) {
				...
				if (readline.contains("[")) {
					...
					String string_raeume = readline.substring(
							readline.indexOf("[") + 1, readline.indexOf("]"));

					String[] split_raeume = string_raeume.split(",");

					// Generierung der Raum Objekte und abspeichern in eine
					// ArrayList
				...
				for (int y = 0; y < raeume.size(); y++) {
					Integer intRaeume = new Integer(raeume.get(y)
							.getRaumnummer());
		...
		for (ArrayList l : data) {
			...
			gebaeude.add(new Gebaeude(object.get(0), object.get(1), object
					.get(2), object.get(3), list_raeume));
